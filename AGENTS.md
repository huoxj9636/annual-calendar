...
- 日期用 `date` 命令确认当天日期，主题用2-4个字概括核心内容
- **同一个上下文 = 同一个文件**：如果当前上下文之前已经创建了 session 文件，继续往里追加；如果没有，新建文件从 Q1/A1 开始
- **禁止扫描 .memory/sessions/ 目录去找已有文件**：只看当前上下文自己的对话记录，不翻目录
...
项目已安装 **neat-freak** 技能（知识治理与收尾规范）。以下场景**必须**加载该技能并按其流程执行：

- 用户说"整理一下"、"收尾"、"同步文档"、"洁癖"、"neat-freak"
- 功能开发完成后需要确保代码、文档、记忆、规则的一致性
- 对话结束前发现文档与代码不一致
- 需要审计工作区残留、清理过期文件

核心原则：
1. **代码为真相源**：文档、记忆与代码冲突时，以代码为准
2. **先减后加**：删除过期内容优先于新增内容
3. **单一权威**：同一事实只保留一个权威版本，其他位置放短指针
4. **记忆谨慎写入**：只记录可复用的教训，不记流水账
5. **破坏性操作需确认**：删除文件、分支等不可逆操作必须经用户确认
