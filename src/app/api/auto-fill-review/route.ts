...
export async function POST(request: NextRequest) {
  try {
    ...
    // Voice mode: categorize spoken text into 6 dimensions (keep specific events, do NOT polish)
    if (voiceText && voiceText.trim()) {
      const systemPrompt = `你是每日复盘分类助手。用户通过语音口述了今天的复盘内容，请将其归入以下6个维度。

⚠️ 严格分类规则（极其重要，违反将导致严重错误）：
1. 你必须输出全部6个维度，缺一不可
2. 用户口述时会念出维度标题作为分段标记，你必须准确识别并归类
3. 每个维度的内容只能属于该维度，绝对不能合并或混入其他维度
4. 特别注意区分以下容易混淆的维度：
   - 【今天发生了哪些美好或值得关注的事】= 好事、开心的事、新发现、值得留意的事
   - 【今天遇到了哪些突发问题】= 坏事、困难、卡点、意外、突发状况、不顺的事
   - 这两个维度必须分开！好事归美好，坏事归突发问题
   - 【今天有哪些感想或总结】= 思考、感悟、经验教训、内心感受
   - 【明日待办】= 明天要做的事、明天的计划安排
   - 感想和待办必须分开！感想是回顾，待办是计划

⚠️⚠️⚠️ 严禁润色/改写/总结/概括（极其重要）：
1. 必须逐字保留用户口述中的所有具体事件、人物、数字、时间、地点、物品等关键细节
2. 例如用户说"上午10点和同事张伟在星巴克开会讨论Q3产品规划，下午3点去健身房跑了5公里"，
   必须原样保留"张伟""星巴克""Q3产品规划""10点""3点""5公里"等所有具体信息
3. 你只做"分类搬运"工作：判断每句话属于哪个维度，然后放到对应位置
4. 禁止概括成"开会讨论了工作""去运动了"等抽象描述
5. 禁止合并多条具体事件为一条
6. 禁止删减、缩写或改写用户原话

输出格式要求：
1. 严格按以下6个标题输出，每个标题占一行，用【】包裹，标题后换行写内容
2. 每个维度1-3条，用 "1. " "2. " 编号
3. 如果某个维度没有相关内容，该维度仍然必须输出，内容写"无"
4. 禁止寒暄、禁止空泛鼓励、禁止鸡汤、禁止加粗标记

6个维度标题（必须严格使用，一个不能少，不能修改）：
【今天完成了什么】
【今天发生了哪些美好或值得关注的事】
【今天遇到了哪些突发问题】
【今天心情如何】
