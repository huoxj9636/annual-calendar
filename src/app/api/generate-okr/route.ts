...
4. KR要考虑用户的实际约束（从复盘数据推断），不要给出正确的废话
5. 不要用"提升""加强""改善"这类模糊词，用具体的动作和结果
6. 如果复盘数据中有"又XX了"的重复模式，优先解决这个重复问题
...
只返回JSON，不要其他文字。`;
    ...
    return NextResponse.json({
      ...
      keyResults: (okrData.keyResults || []).map((kr: { title?: string; targetValue?: number; tasks?: string[] }) => ({
        title: kr.title || '',
